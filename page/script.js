...
const etapas=[{icone:"🧠",titulo:"A ANALISAR AS SUAS RESPOSTAS...",subtitulo:"A processar o seu perfil personalizado"},{icone:"📅",titulo:"A PREPARAR OS 28 DIAS...",subtitulo:"A criar o seu plano de treino exclusivo"},{icone:"🎯",titulo:"A CALCULAR O SEU POTENCIAL...",subtitulo:"A identificar o seu objetivo de rabo ideal"},{icone:"📲",titulo:"QUASE PRONTO!",subtitulo:"A preparar o seu treino personalizado"}];const DURACAO_ETAPA=1800;const TOTAL_ETAPAS=etapas.length;function atualizarChecklist(numeroEtapa){checklistItems.forEach(function(item,index){item.classList.remove('is-done','is-active','is-pending');if(index<numeroEtapa){item.classList.add('is-done');}else if(index===numeroEtapa){item.classList.add('is-active');}else{item.classList.add('is-pending');}});}
